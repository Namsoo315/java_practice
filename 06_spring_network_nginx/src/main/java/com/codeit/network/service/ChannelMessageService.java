...
import com.codeit.network.event.ChannelMessageCreatedEvent;
...
public class ChannelMessageService {
    ...
    public MessageResponse sendChannelMessageFromStomp(
        ...
        // 채널 메시지 생성 이벤트 발행 -> Listener 가 SSE 알림 전송
        eventPublisher.publishEvent(new ChannelMessageCreatedEvent(channelId, response));
        return response;
    }
    ...
    private User getCurrentUserEntity() {
        ...
        return userRepository.findById(dto.id())
                .orElseThrow(() -> new IllegalStateException("DB에서 사용자를 찾을 수 없습니다. id=" + dto.id()));
    }
}
